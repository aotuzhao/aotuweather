        ...
        if (!TextUtils.isEmpty(response)) {
            try {
                ...
                for (int i = 0; i < allProvinces.length(); i++) {
                    JSONObject provinceObjeck = allProvinces.getJSONObject(i);
                    Province province = new Province();
                    province.setProvinceName(provinceObjeck.getString("name"));
                    province.setProvinceCode(provinceObjeck.getInt("id"));
                    province.save();
                }
                return true;
            } catch (JSONException e) {
